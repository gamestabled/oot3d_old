...
n3dsdisasm_output = "baserom/baserom.s"

filePrefix = \
"""\t.include "include/asm/macros.inc"
...
with open(n3dsdisasm_output, 'r') as disasm:
        ...
        if line.find("arm_func_start") >= 0:
            ...
            if currentFunction not in functions_to_not_split:
                if currentFile is not None and not currentFile.closed:
                    currentFile.close()
                currentFile = open("asm/" + currentFunction + ".s", 'w+')
                currentFile.write(filePrefix)
                currentFile.write("\t.section i." + currentFunction + "\n\n\n")
